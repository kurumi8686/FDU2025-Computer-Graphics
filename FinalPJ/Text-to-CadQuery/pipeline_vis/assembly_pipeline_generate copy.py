...
def assembly_text_to_image_pipeline(
        ...
        max_fix_attempts: int = 3  # 新增参数：最大修复尝试次数
):
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for attempt in range(max_fix_attempts + 1):  # 原始尝试 + 修复尝试
            ...
            stl_success, stl_message = run_cadquery_and_save_stl(current_code, temp_stl_path)
            ...
            if stl_success:
                ...
                if attempt < max_fix_attempts:  # 如果还有修复机会
                    print(f"\n尝试使用AI修复代码...")
        ...
        # Copy the rendered image to final path
        try:
...
if __name__ == "__main__":
    ...
    final_image = assembly_text_to_image_pipeline(
        user_text_prompt, model, tokenizer, 
        final_output_image_path=output_file,
        max_fix_attempts=3  # 可以调整最大修复尝试次数
    )
