...
# 如果 evaluate_assembly_pipeline.py 与 assembly_pipeline_generate.py 在同一目录
...
GROUND_TRUTH_STL_DIR = os.path.join(PROJECT_ROOT, 'inference/output_gt')
# 确保真值STL目录 (GROUND_TRUTH_STL_DIR) 中的文件已通过运行 run_step2_gt.py 生成

# --- Metric Calculation Functions (adapted from inference/step5_compute_metrics/run5.py) ---
def sample_mesh_normalized(stl_path, num_points=10000):
    try:
        mesh = trimesh.load(stl_path, force='mesh')
        if not (mesh.vertices.shape[0] > 0 and mesh.faces.shape[0] > 0):
            # print(f"Warning: Mesh {os.path.basename(stl_path)} is empty or invalid for sampling.")
            return None
        mesh.apply_translation(-mesh.centroid)
        scale = np.max(mesh.bounding_box.extents)
        if scale == 0: # Avoid division by zero for degenerate meshes
            # print(f"Warning: Mesh {os.path.basename(stl_path)} has zero extent after centering.")
            return None
        mesh.apply_scale(1.0 / scale)
        points, _ = trimesh.sample.sample_surface(mesh, num_points)
        return points
    except Exception as e:
        # print(f"Error sampling mesh {stl_path}: {e}")
        return None
...
def normalize_mesh(mesh):
    mesh = mesh.copy()
    if not (mesh.vertices.shape[0] > 0 and mesh.faces.shape[0] > 0):
        return mesh # Return original if empty
    bounds = mesh.bounds
...
def voxelize_mesh(mesh, voxel_size=0.02):
    if not (mesh.vertices.shape[0] > 0 and mesh.faces.shape[0] > 0):
        return np.array([], dtype=bool) # Return empty if mesh is invalid
    try:
...
def volumetric_iou(mesh1_path, mesh2_path, voxel_size=0.02):
    try:
        ...
        if not (mesh1.vertices.shape[0] > 0 and mesh1.faces.shape[0] > 0 and \
            ...
            # print(f"Warning: One or both meshes are empty/invalid for IoU ({os.path.basename(mesh1_path)}, {os.path.basename(mesh2_path)}).")
            return np.nan
        ...
        if vox1.size == 0 or vox2.size == 0:
            # print(f"Warning: Voxelization failed for IoU ({os.path.basename(mesh1_path)}, {os.path.basename(mesh2_path)}).")
            return np.nan
        ...
        # print(f"Error calculating IoU for {mesh1_path} and {mesh2_path}: {e}")
        return np.nan

# --- Evaluation Functions ---
def generate_predicted_stls(model, tokenizer):
    print("--- Part 1: Generating Predicted STLs using Assembly Pipeline ---")
        ...
        for line_number, line_content in enumerate(f_in):
            ...
            try:
                ...
                if "input" in data:
                    ...
                    print(f"Skipping line {line_number + 1}: 'input' key not found.")
            except json.JSONDecodeError as e:
                print(f"Skipping line {line_number + 1} due to JSON decoding error: {e}")
            
            if len(prompts_to_process) >= max_prompts:
                print(f"Limited to first {max_prompts} prompts for faster evaluation.")
                break

    print(f"Will process {len(prompts_to_process)} prompts (limited to first {max_prompts}).")
    ...
    for idx, user_prompt in enumerate(tqdm(prompts_to_process, desc="Generating Predicted STLs")):
        ...
        try:
            _, generated_stl_path = assembly_text_to_image_pipeline(
                ...
                max_fix_attempts=2, # Or your preferred value
            ...
            if generated_stl_path and os.path.exists(generated_stl_path) and os.path.getsize(generated_stl_path) > 0:
                ...
                # Create an empty file to maintain numbering for metrics script if needed, or handle missing files in metrics
                # open(target_pred_stl_path, 'w').close()


            if os.path.exists(dummy_image_output_path):
                os.remove(dummy_image_output_path) # Clean up dummy image

        except Exception as e:
            print(f"Error processing prompt {idx} with assembly pipeline: {e}")
            traceback.print_exc()
            # Create an empty file if error before STL generation
            # open(target_pred_stl_path, 'w').close()
...
def compute_all_metrics():
    ...
    if not os.path.exists(GROUND_TRUTH_STL_DIR):
        ...
        print("Please ensure you have run the script to generate ground truth STLs (e.g., run_step2_gt.py).")
        return
    ...
    for filename in tqdm(common_files, desc="Computing Metrics"):
        ...
        # Skip if predicted STL is empty (might have been created as a placeholder)
        if os.path.exists(pred_stl_path) and os.path.getsize(pred_stl_path) == 0:
            print(f"Skipping {filename}: Predicted STL is empty.")
            error_files_metric.append(filename + " (empty predicted STL)")
            cd_results.append(np.nan)
            f1_results.append(np.nan)
            iou_results.append(np.nan)
            continue
        if not os.path.exists(pred_stl_path) :
            print(f"Skipping {filename}: Predicted STL does not exist.")
            error_files_metric.append(filename + " (missing predicted STL)")
            cd_results.append(np.nan)
            f1_results.append(np.nan)
            iou_results.append(np.nan)
            continue
        ...
        if points_gt is None or points_pred is None:
            ...
            cd_results.append(np.nan)
            f1_results.append(np.nan)
            # IoU is calculated differently, handle its potential errors inside volumetric_iou
        else:
        ...
        iou = volumetric_iou(pred_stl_path, gt_stl_path) # Note: volumetric_iou takes paths
        iou_results.append(iou)


    print("\n--- Metric Results ---")
    # Filter out NaNs for mean/median calculation
    ...
    if error_files_metric:
        ...
        # for f_err in error_files_metric:
        #     print(f" - {f_err}")
...
def main():
    ...
    try:
        tokenizer = AutoTokenizer.from_pretrained(
            ...
            use_fast=False,
            model_max_length=1024 # Or your model's specific max length
        )
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.padding_side = "left" # Important for some models
        model = AutoModelForCausalLM.from_pretrained(
            HF_MODEL_PATH,
            trust_remote_code=True,
            torch_dtype=torch.bfloat16, # Or torch.float16 if bf16 not available
            device_map="auto"
        )
        model.eval()
    ...
    # --- Step 1: Generate Predicted STLs ---
    generate_predicted_stls(model, tokenizer)

    # --- Step 2: Compute Metrics ---
    # (Ensure ground truth STLs are already generated in GROUND_TRUTH_STL_DIR by run_step2_gt.py)
    compute_all_metrics()
